...
bool preRequisitoFraco(ALUNOMATRICULADO* a1) {
	...
	if(a1->nota >= 30 && a1->frequencia >= 70){
        return true;
	}
	return false;
}
...
bool atualizarNotaEFrequencia(ALUNOMATRICULADO* a1, int nota, int freq) {
	...
	/* COMPLETE/IMPLEMENTE SEU CODIGO AQUI */
    if(nota < 0 || freq < 0 || nota > 100 || freq > 100){
        return false;
    }
    a1->nota = nota;
    a1->frequencia = freq;
...
void atualizarStatus(ALUNOMATRICULADO* a1) {
	...
	/* COMPLETE/IMPLEMENTE SEU CODIGO AQUI */
    if(a1->nota < 50 && a1->frequencia < 70){
        a1->status = 'E';
    }
    else if(a1->nota < 30 && a1->frequencia >= 70){
        a1->status = 'N';
    }
    else if(a1->nota >= 50 && a1->frequencia < 70){
        a1->status = 'F';
    }
    else if( (a1->nota >= 30 && a1->nota < 50) && a1->frequencia >= 70){
        a1->status = 'R';
    }
    else if(a1->nota >= 50 && a1->frequencia >= 70){
        a1->status = 'A';
...
void arredondarParaCinco(MINISTRANTE min1, int valorMinimo) {
    ...
    for (int i = 0 ; i < min1.numAlunos ; i++){
        if (!(min1.alunos[i]->frequencia < 70)){
            if (!(min1.alunos[i]->nota >= 50 || min1.alunos[i]->nota < valorMinimo)){
                min1.alunos[i]->nota = 50;
            }
        }
    }
}
...
bool adicionarMinistrante(CURSO* c1, MINISTRANTE* min1){
    ...
    for(int i = 0; i < c1->numMinistrantes; i++){
            if(c1->ministrantes[i]->pessoa.nusp == min1->pessoa.nusp){
                return false;
            }
    }
